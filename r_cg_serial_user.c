...
{
    if(UartState_enm == UART_TX_START)
    {
	    if(Uart.NoOfBytesToTransmit_u8)
	    {
		    TXD0 = *Uart.UartTxPtr++;
		    Uart.NoOfBytesToTransmit_u8--;    
	    }
	    else if(Uart.NoOfBytesToTransmit_u8 == 0)
	    {
		UartState_enm = UART_TX_COMPLETED;    
	    }
    }
    
}

void InitiateUartTransmit(unsigned char * TxPtr, unsigned char NoOfBytes)
{
	Uart.UartTxPtr = TxPtr;
	Uart.NoOfBytesToTransmit_u8 = (NoOfBytes-1)  ;
	UartState_enm = UART_TX_START;
	TXD0 = *Uart.UartTxPtr++;
}
/***********************************************************************************************************************
